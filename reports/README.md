...
When we commit and push code to our GitHub repo, it automatically triggers our CI/CD pipeline via GitHub Actions. The CI workflow runs 17 unit tests (across data, model, and API), linting with ruff, and code coverage reporting across three operating systems (Ubuntu, Windows, and macOS). Simultaneously, the Docker workflow builds containerized images and pushes them to the Google Container Registry.
